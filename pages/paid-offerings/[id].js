...
function PaidOfferingDetail({ offering }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-[2rem]">
          ...
          <div className="md:col-span-1 w-full flex flex-col items-center gap-[1rem]">
            <div className="flex flex-col gap-[10px] w-full md:w-[90%] py-[2rem] bg-primaryColor z-10 sticky top-[3rem]">
              ...
              {role !== "Visitor" && (
                <button
                  className="btn w-full rounded-[0px] hover:shadow-xl"
                  onClick={handleFollow}
                >
                  {follow ? "Unfollow this offering" : "Follow this offering"}
                </button>
              )}
            </div>
...
export default PaidOfferingDetail;

export async function getServerSideProps({ params }) {
  const offeringId = params.id;

  try {
    const res = await fetch(`http://localhost:3001/paidOffering/${offeringId}`);
