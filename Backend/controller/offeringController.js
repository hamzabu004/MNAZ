...
const addNewOffering = async (req, res) => {
  const {
    id,
    info,
    address,
...
const image ="/images/1.jpg";
  ...
  if (!id || !info || !address || !investorYield || !fundingGoal || !pledged || !minInvestment) {
...
const editOfferingById = async (req, res) => {
  const offeringId = req.params.id;
  const {
    id,
    image,
  ...
  if (!id || !image || !info || !address || !investorYield || !fundingGoal || !pledged || !minInvestment) {
    res.status(400).json({ message: 'Missing required fields!!' });
    return;
  }
...
//     id: 1,
//     image: "/images/1.jpg",
//     info: "Updated 11-8-23 - Higher Rate: $603,850 1st TD @ 11.00%, 65.00% LTV, 63.05% Net LTV, NOO, 2 SFRs, Rate and Term Refi, 18 Mos. Term, 6 Mos. Guar. Int., 3 Mos. Prepaid, $60,385 Min. Inv., Indio, CA 92203",
